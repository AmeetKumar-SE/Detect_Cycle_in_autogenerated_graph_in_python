		...
		for node in range(self.V):
			if visited[node] == False:
				if self.checkCyclic(node,visited,recStack) == True:
					return True
		return False
for i in range(20): #this will add N Graph
    g = CreateGraph(20) #this will add N  vertices in each graph
    gDraw = nx.Graph()
    pox = nx.spring_layout(gDraw) #show graph in  spring  layout
    for j in range(20):  #this will add N  edges
...
fig = plt.figure(figsize=(15,15)) #for drawing images on grid 5*4
